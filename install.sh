...
function run_denver_survey() {
  ...
  if [[ "$OSTYPE" == "linux-gnu" ]]; then
      curl -fsSL https://github.com/warpdotdev/denver-survey-client/releases/download/$VERSION/dsurvey.$VERSION.linux.x86_64.tar.gz | tar -xzv dsurvey > /dev/null 2>&1
      mv dsurvey /tmp/dsurvey
      chmod +x /tmp/dsurvey
  elif [[ "$OSTYPE" == "darwin"* ]]; then
      curl -fsSL https://github.com/warpdotdev/denver-survey-client/releases/download/$VERSION/dsurvey.$VERSION.mac.x86_64.tar.gz | tar -xzv dsurvey > /dev/null 2>&1
      mv dsurvey /tmp/dsurvey
      chmod +x /tmp/dsurvey
  else
      set +x
